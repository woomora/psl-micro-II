...
.pull-left[
```{r, out.width="65%"}
knitr::include_graphics("http://storage.googleapis.com/passculture-metier-prod-production-assets-fine-grained/thumbs/mediations/4PAMS")
```
]

.pull-right[
```{r, out.width="70%"}
knitr::include_graphics("https://radical-production.fr/uploads/divers/res2026-annonce1-lineup-meta-1080x1440.jpg")
```
]
...
```{r, out.width="40%"}
knitr::include_graphics("https://www.parisladefense-arena.com/uploads/2025/05/visu-date-2-819x1024.jpg")
```
...
```{r, out.width="85%"}
knitr::include_graphics("imgs/billets-bad-bunny.png")
```
...
plot_capturing_cs +
  ...
  annotate(
    ...
    label = expression(P[1]),  # Using expression for dynamic value
...
plot_imperfect_pd +
  ...
  annotate(
    ...
    label = expression(P[2]),  # Using expression for dynamic value
    fontface = "italic", size = 3, hjust = -0.85
  )
...
plot_imperfect_pd +
  ...
  annotate(
    ...
    label = expression(P[3]),  # Using expression for dynamic value
    fontface = "italic", size = 3, hjust = -0.85
  )
...
plot_imperfect_pd +
  ...
  annotate(
    ...
    label = expression(P[4]),  # Using expression for dynamic value
    fontface = "italic", size = 3, hjust = -0.85
  )
...
plot_imperfect_pd +
  ...
  annotate(
    ...
    label = expression(P[5]),  # Using expression for dynamic value
    fontface = "italic", size = 3, hjust = -0.85
  )
...
plot_imperfect_pd +
  ...
  annotate(
    ...
    label = expression(P[6]),  # Using expression for dynamic value
    fontface = "italic", size = 3, hjust = -0.85
  )
...
plot_3rd_pd <- plot_3rd_pd +
  ...
  annotate(
    "text",
    x = eq_q, y = 0,  
    label = expression(Q["T"] == .(eq_q)),  # Using expression for dynamic value
...
plot_3rd_pd <- plot_3rd_pd +
  ...
  annotate(
    "text",
    x = eq_q1, y = 0,  
    label = expression(Q["1"] == .(eq_q1)),  # Using expression for dynamic value
    fontface = "italic", size = 2.4, hjust = -0.15
  )
...
plot_3rd_pd <- plot_3rd_pd +
  ...
  annotate(
    "text",
    x = eq_q2, y = 0,  
    label = expression(Q["2"] == .(eq_q2)),  # Using expression for dynamic value
    fontface = "italic", size = 2.4, hjust = -0.15
  )
...
plot_3rd_pd <- plot_3rd_pd +
  ...
  annotate(
    "text",  
    x = 0, y = eq_p1,  
    label = expression(P["1"] == .(eq_p1)),  # Using expression for dynamic value
...
plot_3rd_pd <- plot_3rd_pd +
  ...
  annotate(
    "text",  
    x = 0, y = eq_p2,  
    label = expression(P["2"] == .(eq_p2)),  # Using expression for dynamic value
    fontface = "italic", size = 2.5, vjust = -.75  # Adjust size and alignment
  )
...
.pull-right[
$$\Rightarrow \; \Delta P_2= -3 \cdot \Delta Q_2$$
]
--
.pull-left[
...
--
.pull-left[
$$\Rightarrow \; \frac{1}{ \varepsilon_{d1}}= - 0.75 \cdot \frac{13.7}{29.7} \approx -0.346$$
]
.pull-right[
$$\Rightarrow \; \frac{1}{ \varepsilon_{d2}}= - 3 \cdot \frac{6.8}{39.6} \approx -0.515$$
]
--

$$\frac{P_1}{P_2} = \frac{1 + \frac{1}{\varepsilon_{d2}}}{1 + \frac{1}{\varepsilon_{d1}}} \; \Rightarrow  \; \frac{1-0.515}{1-0.346} \approx 0.75$$
...
plot_intmp_price <- 
  ...
  annotate(
    "text",
    x = eq_q2, y = 0,  
    label = expression(Q["1"] == .(eq_q2)),  # Using expression for dynamic value
    fontface = "italic", size = 2.4, hjust = -0.15
  ) +
  geom_segment(aes(x = 0, xend = eq_q2, y = eq_p2, yend = eq_p2), linetype = "dashed", color = "gray50") +
  annotate(
    "text",
    x = 0, y = eq_p2,  
    label = expression(P["1"] == .(eq_p2)),  # Using expression for dynamic value
    fontface = "italic", size = 2.4, vjust = -0.15
  ) +
  geom_point(
...
plot_2p_tariff <- 
  ...
  annotate(
    "text",
    x = 20, y = 22,  
    label = expression(P^"*" == MC),  # Using expression
